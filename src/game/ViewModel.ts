...
export class ViewModel {
  ...
  private add(geo: THREE.BufferGeometry, color: number, emissive: number): THREE.Mesh {
  ...
  private buildRifle(): void {
    ...
    stock.position.set(0, 0, 0.12);         // z+ = 카메라 쪽 (화면 가장자리)

    // 리시버 (총 몸통 중앙)
    const recv = this.add(new THREE.BoxGeometry(0.038, 0.074, 0.28), metal, mE);
    recv.position.set(0, 0.004, -0.16);

    // 탄창
    const mag = this.add(new THREE.BoxGeometry(0.026, 0.118, 0.062), metal, mE);
    mag.position.set(0, -0.088, -0.13);

    // 방아쇠 울
    const guard = this.add(new THREE.BoxGeometry(0.008, 0.040, 0.065), metal, mE);
    ...
    const barrel = this.add(barrelGeo, metal, mE);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.set(-0.010, -0.007, -0.66);

    // 총구
    const muzzleGeo = new THREE.CylinderGeometry(0.015, 0.013, 0.036, 6);
    const muzzle = this.add(muzzleGeo, metal, mE);
    muzzle.rotation.x = Math.PI / 2;
    muzzle.position.set(-0.010, -0.007, -0.94);

    // 가늠쇠 (front sight)
    const fSight = this.add(new THREE.BoxGeometry(0.014, 0.024, 0.015), metal, mE);
    fSight.position.set(-0.010, 0.018, -0.91);

    // 가늠자 (rear sight)
    const rSight = this.add(new THREE.BoxGeometry(0.016, 0.018, 0.010), metal, mE);
